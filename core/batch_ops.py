...
from utils.file_utils import get_safe_child_path
...
class BatchOps:
    ...
    def batch_create_folders(
        ...
        tx = manager.create_transaction("批量创建文件夹", f"根路径: {base_path}")

        for idx, name in enumerate(clean_names):
            if token and token.is_cancelled:
                break
            try:
                target_dir = get_safe_child_path(base_path, name)
    ...
    def preview_batch_rename(
        ...
        for old, new in name_pairs:
            ...
            try:
                old_path = get_safe_child_path(base_path, final_old)
            except ValueError as exc:
                status = f"原名称无效: {exc}"

            if status == "可重命名":
                try:
                    new_path = get_safe_child_path(base_path, final_new)
                except ValueError as exc:
                    status = f"目标名称无效: {exc}"
    ...
    def execute_batch_rename(
        ...
        manager = undo_manager or undo_mgr
        tx = manager.create_transaction("批量重命名", f"路径: {base_path}")
    ...
    def batch_safe_delete(
        ...
        for idx, name in enumerate(clean_names):
            if token and token.is_cancelled:
                break
            try:
                target_path = get_safe_child_path(base_path, name)
